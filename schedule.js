...
export function computeDailyCapacity(team, startDate, endDate){
  ...
  while(cur <= end){
    if(!isWeekend(cur)){
      ...
      team.memberAssignments.forEach(a=>{
        if(!a || !a.specialty || !a.startDate) return;
        const s = new Date(a.startDate);
        const e = a.endDate ? new Date(a.endDate) : null;
        if(s <= cur && (!e || cur < e)){
          dayCap[a.specialty] = (dayCap[a.specialty]||0) + 1;
        }
      });
...
export function getCapacityForDateRange(capacitySegments, startDate){
  ...
  const s = capacitySegments.find(seg => seg.date <= startDate) || capacitySegments[0];
  return s ? s.capacity : {};
}
...
export function aggregate(plan, tasks, getTeam, targetDate = null){
  ...
  if(team && Array.isArray(team.memberAssignments) && team.memberAssignments.length){
    ...
    capacitySegments = computeDailyCapacity(team, earliest, latest);
